...
## 4. Открытое
- Название проекта временное. Ёмкость гекса 4 бойца (`FlowLogic.Cap`, автор 14.09); бойцы 0.34×0.64, слоты 2×2 в гексе (`Field3D.SlotOffset/SlotOf`), лагерь на старте — `FlowLogic.InitFighters`. Кисть (зона под пальцем шире линии, правило 40 %) из Warbands сюда не перенесена — линия по гексам между касаниями. Красный контур рейнджа удара не рисуется, только затемнение радиуса хода.
- Баланс на новых правилах — прогоны в BALANCE §1–§4 (актуальный — §4: cap 4, составы прежние); скорости 5/6/3 и HP героев не трогались по воле автора. Bone Tide берёт до 11 ульт за бой — посмотреть.
