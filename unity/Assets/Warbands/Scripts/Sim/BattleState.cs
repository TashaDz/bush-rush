...
namespace Warbands.Sim
{
    ...
    public sealed class Unit
    {
        ...
        public List<Cell> Fighters;   // Bush Rush «как вода»: гекс каждого бойца; Cell — передний боец
        public List<int> FighterHp;   // HP каждого бойца (сумма = Hp, каждый ≤ HpPerFighter): кто умирает — решает расстояние до атакующего   // ветка bush-field: клетка и цель хода (враг/союзник или бонус на клетке)
        ...
        public Unit Clone()
        {
            ...
            if (FighterHp != null) u.FighterHp = new List<int>(FighterHp);
