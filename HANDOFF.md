...
## 4. Открытое
- Название проекта временное. Ёмкость гекса 16 бойцов (`FlowLogic.Cap`) — число предложено. Кисть (зона под пальцем шире линии, правило 40 %) из Warbands сюда не перенесена — линия по гексам между касаниями. Красный контур рейнджа удара не рисуется, только затемнение радиуса хода.
